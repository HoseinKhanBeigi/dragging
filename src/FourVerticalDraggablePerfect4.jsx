...
export default function FourVerticalDraggable() {
  ...
  const isStackClosed = boxes.every((y) => y <= BASE_Y + 4);
  ...
  return (
    ...
    >
      {boxes.map((y, i) => {
        ...
        const visibleAmount = isVisibleCard ? 1 : lift;
        const z = isStackClosed ? -180 + i * 28 : 0;
        const rotateX = isStackClosed ? 16 - i * 1.4 : 0;
        const closedPeek = isStackClosed && isVisibleCard
          ? stackRank * 14
          : 0;
        const closedScale = isStackClosed && isVisibleCard
          ? 1 - stackRank * 0.06
          : 1;
